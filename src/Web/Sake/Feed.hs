...
renderFeed tmpl conf@FeedConf{..} itemCtx is = do
  let itemCtx' = mconcat [ itemCtx
                         , field "updated" $ fmap formatForAtomDate . itemUpdatedDate
                         , bodyField "description"
                         ]
  items <- mapM (runContext itemCtx') is
  let Object dic = toJSON conf
      upd = L.maximumBy (comparing zonedTimeToUTC) $
            mapMaybe (parseTimeM @Maybe @ZonedTime True defaultTimeLocale "%Y-%m-%dT%H:%M:%S%z"
                      <=< maybeResult . fromJSON
                      <=< HM.lookup "updated") items
      obj = HM.fromList [("items", toJSON items), ("updated", toJSON upd)]
